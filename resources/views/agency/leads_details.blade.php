                            ...
                            <div class="new-property">
                                ...
                                <div class="row no-gutters position-relative buy_leads_div">
                                    <div class="col-12 mb-3">
                                        ...
                                        <p>Create Date: &emsp;<strong>{{date('d M, Y',strtotime($data['row']->CREATED_AT))}}</strong></p>
                                        <p>Lead Type: &emsp;<strong>
                                                @if($data['row']->LEAD_TYPE == 0)
                                                    100% Matching
                                                @else
                                                    Force Lead
                                                @endif
                                            </strong></p>
                                        <p>Lead Id: &emsp;<strong>{{$data['row']->CODE}}</strong></p>
                                    </div>
                                    @php
                                        $area= [];
                                        foreach(json_decode($data['row']->getRequirements->F_AREAS) as $i){
                                            $area[] = \App\Models\Area::where('id',$i)->first()->AREA_NAME;
                                        }
                                    @endphp
                                    <div class="col-12 mb-3">
                                        <h3 class="mb-1">Requirement Details</h3>
                                        <p>Looking property for: <strong>{{$data['row']->getRequirements->PROPERTY_FOR}}</strong></p>
                                        <p>Property Type: &emsp;<strong>{{$data['row']->getRequirements->PROPERTY_TYPE}}</strong></p>
                                        <p>Preferred Location: &emsp;<strong>{{implode(', ',$area)}}
                                            </strong></p>
                                        <p>Property Size: &emsp;<strong>{{$data['row']->getRequirements->MIN_SIZE}}
                                                - {{$data['row']->getRequirements->MAX_SIZE}} sft</strong></p>
                                        <p>Condition: &emsp;<strong>
                                                @php
                                                    $c =[];
                                                @endphp
                                                @foreach (json_decode($data['row']->getRequirements->PROPERTY_CONDITION) as $cond) @php  $c[] = $cond @endphp @endforeach {{implode(', ',$c)}}
                                            </strong></p>
                                        <p>Requirement Details: &emsp;
                                            <strong>
                                                {!! $data['row']->getRequirements->REQUIREMENT_DETAILS !!}
                                            </strong>
                                        </p>
                                        <p>Preferred Time To Contact: &emsp;<strong>{{$data['row']->getRequirements->PREP_CONT_TIME}}</strong></p>
                                    </div>

                                    <div class="col-12 mb-2">
                                        <h3 class="mb-1">Seeker Details</h3>
                                        <p>Name: &emsp;<strong>{{$data['row']->getRequirements->getUser->NAME}}</strong></p>
                                        <p>Country: &emsp;<strong>Bangladesh</strong></p>

                                        @if($data['row']->STATUS == 1)
                                        <p>Mobile: &emsp;<strong>{{$data['row']->getRequirements->getUser->MOBILE_NO}}</strong></p>
                                        <p>Email: &emsp;<strong>{{$data['row']->getRequirements->getUser->EMAIL}}</strong></p>
                                        @endif

                                        @if($data['row']->STATUS == 0)
                                            <p class="text-danger">Mobile number & Email Address only Could be view if you buy this
                                                BDT {{$data['row']->getRequirements->LEAD_PRICE}}</p>
                                        @endif
                                    </div>
                                    ...
                                    @endif
                                    @if($data['row']->STATUS == 1)
                                    <p>Purchased At : {{ date('d M Y, h:i A', strtotime($data['row']->PURCHASE_AT)) }}</p>
            ...
            <div class="modal-content">
                <div class="modal-body">
                    <div class="login-wrap text-center">
                        @auth
                            {!! Form::open([ 'route' => ['developer.lead.pay',$data['row']->id], 'method' => 'post', 'id'=>'login_user', 'class' => 'form-horizontal', 'files' => true , 'novalidate', 'autocomplete' => 'off']) !!}

                            {!! Form::hidden('f_lead_share_no',$data['row']->id) !!}
                            {!! Form::hidden('f_requirement_no',$data['row']->F_REQUIREMENT_NO) !!}
                            {!! Form::hidden('price',$data['row']->getRequirements->LEAD_PRICE) !!}

                            <div class="row">
                                @if(Auth::user()->UNUSED_TOPUP < $data['row']->getRequirements->LEAD_PRICE )
                                    <div class="col-12">
                                ...
                                @else
                                    <div class="col-12">
                                        ...
                                        <p>Please Pay:</p>
                                        <strong
                                            style="font-size: 28px;color: #FF3521;display: inline-block;margin-bottom: 25px !important;">BDT {{ number_format($data['row']->getRequirements->LEAD_PRICE,2) }}</strong>
                                        <p>Your current balance</p>
                                        <strong
